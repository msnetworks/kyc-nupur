...
@section('admin-content')
                        ...
                        <div class="d-flex flex-column mb-3">
                            ...
                            <div class="observer-filter-grid observer-date-filter">
                                <div class="form-group">
                                    ...
                                    <input id="observer-start-date" type="date" class="form-control"
                                        value="{{ request('start_date') }}">
                                </div>
                                <div class="form-group">
                                    <label class="small text-muted">End date</label>
                                    <input id="observer-end-date" type="date" class="form-control"
                                        value="{{ request('end_date') }}">
                                </div>
                                <div class="form-group">
                                    <label class="small text-muted">Action</label>
                                    <button id="observerApplyFilters" type="button"
                            ...
                            <div class="observer-filter-grid observer-search-row mt-3">
                                <div class="form-group">
                                    ...
                                    <input id="observer-search" type="text" class="form-control"
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="observer-per-page" class="custom-select">
                                        @foreach ($perPageOptions as $value)
                                            <option value="{{ $value }}"
                                                {{ (int) $perPage === $value ? 'selected' : '' }}>{{ $value }} per
                                                page</option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <div class="modal fade" id="observerCpvRemarksModel" tabindex="-1" role="dialog"
        aria-labelledby="observerCpvRemarksModelLabel">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group col-md-12 col-sm-12">
                        ...
                        <div id="observer_cpv_comments"
                            style="background-color: #f5f5f5; padding: 10px; border-radius: 4px; min-height: 100px; max-height: 300px; overflow-y: auto;">
    ...
    <!-- CPV Remarks Modal -->
    <div class="modal fade" id="cpvRemarksModel" tabindex="-1" role="dialog" aria-labelledby="cpvRemarksModelLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">CPV Comments</h5>
                ...
                <div class="modal-body">
                    <input type="hidden" id="cpv_case_id">
                    <div class="form-group col-md-12 col-sm-12">
                        <label><strong>Previous Comments:</strong></label>
                        <div id="cpv_previous_comments" style="background-color: #f5f5f5; padding: 10px; border-radius: 4px; min-height: 80px; max-height: 200px; overflow-y: auto; margin-bottom: 15px;">
                            <p style="color: #999;">No previous comments</p>
                        </div>
                    </div>
                    <div class="form-group col-md-12 col-sm-12">
                        <label for="cpv_new_comment"><strong>Add New Comment:</strong></label>
                        <textarea id="cpv_new_comment" class="form-control" rows="4" placeholder="Enter your comment here..."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" id="cpv_submit_btn" class="btn btn-success">Save Comment</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).on('click', '.observerCpvRemarks', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Display comments
                    if (response.case_fi_type && response.case_fi_type.app_remarks && response
                        .case_fi_type.app_remarks.trim() != '') {
                        $('#observer_cpv_comments').html('<p style="white-space: pre-wrap;">' + response
                            .case_fi_type.app_remarks + '</p>');
                    } else {
                        $('#observer_cpv_comments').html(
                ...
                error: function() {
                    $('#observer_cpv_comments').html(
                        '<p style="color: #d32f2f;">Error loading comments. Please try again.</p>');
                    $('#observerCpvRemarksModel').modal('show');
                }
            });
        });
        ...
        <!-- CPV Remarks Handler -->
    <script>
        $(document).on('click', '.cpvRemarks', function() {
            let caseId = $(this).data('row');
            $('#cpv_case_id').val(caseId);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.case_fi_type && response.case_fi_type.app_remarks && response
                        .case_fi_type.app_remarks.trim() != '') {
                        $('#cpv_previous_comments').html('<p style="white-space: pre-wrap;">' + response
                            .case_fi_type.app_remarks + '</p>');
                    } else {
                        $('#cpv_previous_comments').html(
                            '<p style="color: #999;">No previous comments</p>');
                    }
        ...
        // Save CPV remarks
        $(document).on('click', '#cpv_submit_btn', function(e) {
            e.preventDefault();

            let caseId = $('#cpv_case_id').val();
            let newComment = $('#cpv_new_comment').val().trim();
            let previousComment = $('#cpv_previous_comments').text();
            ...
            // Combine old and new comments
            let previousText = previousComment.trim();
            if (previousText === 'No previous comments') {
                previousText = '';
            }

            let updatedRemarks = newComment;


            // Send AJAX request
            $.ajax({
                url: "{{ route('admin.case.updateCpvRemarks') }}",
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    case_id: caseId,
                    app_remarks: updatedRemarks
                },
                success: function(response) {
                    if (response.success) {
                        alert('Comments saved successfully!');
                        $('#cpvRemarksModel').modal('hide');
                        location.reload();
                    } else {
                        alert('Error: ' + (response.message || 'Unable to save comments'));
                    }
                },
