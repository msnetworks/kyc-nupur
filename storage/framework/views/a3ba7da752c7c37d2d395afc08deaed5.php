                ...
                <div class="card-body p-0">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="pull-right">
                            ...
                            <a class="btn btn-warning text-white" href="<?php echo e(route('admin.case.export.excel',['status' => $status, 'user_id' => $user_id])); ?>">Export Cases</a>
                        </div>
                        </div>
                    </div>
                    <div class="row">
                    ...
                    <div id="caseTableContainer">
                        <?php echo $__env->make('backend.pages.cases.caseTable', ['cases' => $cases, 'assign' => $assign], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    </div>
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="<?php echo e(route('admin.case.assignAgent')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group col-md-12 col-sm-12">
                        ...
                        <input name="ScheduledVisitDate" class="custom-select" value="<?php echo e(date('Y-m-d H:i:s')); ?>" id="ScheduledVisitDate" type="datetime-local">
                    </div>
                </div>
...
<div class="modal fade" id="resolveCaseModel" tabindex="-1" role="dialog" aria-labelledby="resolveCaseModelLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="<?php echo e(route('admin.case.resolveCase')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="modal-header">
...
<div class="modal fade" id="verifiedCaseModel" tabindex="-1" role="dialog" aria-labelledby="verifiedCaseModelLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="<?php echo e(route('admin.case.verifiedCase')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="modal-header">
...
<div class="modal fade" id="caseReinitiatesModel" tabindex="-1" role="dialog" aria-labelledby="caseReinitiatesModelLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="<?php echo e(route('admin.case.reinitatiate.store')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="modal-header">
            ...
            <div class="modal-body">
                ...
                <div class="form-group col-md-12 col-sm-12">
                    ...
                    <select name="fi_type" id="fi_type" class="form-control">
                        ...
                        <?php $__currentLoopData = $fitype; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="modal-body">
                ...
                <div class="form-group col-md-12 col-sm-12">
                    ...
                    <select name="branch_code" id="bc_branch_code" class="form-control">
                        ...
                        <?php $__currentLoopData = $branchcode; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($item->id); ?>"><?php echo e($item->branch_name.' ('.$item->branch_code.')'); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div class="form-group col-md-12 col-sm-12">
...
<script>
    ...
    $(document).on('click', '.productModel', function(e) {
        ...
        var url = "<?php echo e(route('admin.case.reinitatiateCaseNew','CASE_ID')); ?>";
...
<script>
    ...
    $('#updateFiTypeButton').on('click', function(e) {
        ...
        var url = "<?php echo e(route('admin.case.fitype','CASE_ID')); ?>";
    ...
    $('#updateProductButton').on('click', function(e) {
        ...
        var url = "<?php echo e(route('admin.case.product','CASE_ID')); ?>";
        url = url.replace('CASE_ID', caseTableId);
        // Send AJAX request
        $.ajax({
            url: url,
    ...
    $('#bc_updateFiTypeButton').on('click', function(e) {
        ...
        var url = "<?php echo e(route('admin.case.branchcode','CASE_ID')); ?>";
        url = url.replace('CASE_ID', caseTableId);
        // Send AJAX request
        $.ajax({
            url: url,
...
<script>
    $(document).on('click', '.resolveCase', function() {
        ...
        let customGetPath = "<?php echo e(route('admin.users.caseStatus', ['type' => 1])); ?>";
        $.ajax({
            url: customGetPath,
            type: 'GET',
            success: function(response) {
...
<script>
        $(document).on('click', '#getSelectedIds', function() {
            ...
            if (selectedIds.length > 0) {
                ...
                var customGetPath = "<?php echo e(route('admin.users.agent','1')); ?>";
                $.ajax({
                    url: customGetPath,
                    type: 'GET',
                    success: function(response) {
...
<script>
        ...
        $(document).on('click', '.assignSingle', function() {
            ...
            let customGetPath = "<?php echo e(route('admin.users.agent','1')); ?>";
...
<script>
        $(document).on('click', '.verifiedCase', function() {
            let getRow = $(this).attr('data-row');
            let customGetPath = "<?php echo e(route('admin.users.caseStatus', ['type' => 1, 'parent_id' => '4'])); ?>";
            $.ajax({
                url: customGetPath,
                type: 'GET',
                success: function(response) {
...
<script>
        $(document).on('change', '#feedbackSelect', function() {
            ...
            if (parent_id.length > 0) {
                var url = "<?php echo e(route('admin.users.caseStatus', ['type' => 1, 'parent_id' => 'PARENT_ID'])); ?>";
...
<script>
        $(document).on('click', '.consolidatedRemarks', function() {
            ...
            var url = "<?php echo e(route('admin.case.getCase','CASE_ID')); ?>";
...
<script>
        $(document).on('click', '.caseHistory', function() {
            let case_id = $(this).attr('data-row');
            var url = "<?php echo e(route('admin.case.getcaseHistory','CASE_ID')); ?>";
            url = url.replace('CASE_ID', case_id);
...
<script>
        $(document).on('click', '.caseClose', function() {
            if (confirm('Are you sure to close this case')) {
                let case_id = $(this).attr('data-row');
                var url = "<?php echo e(route('admin.case.close','CASE_ID')); ?>";
                url = url.replace('CASE_ID', case_id);
...
<script>
        $(document).on('click', '.viewFormEdit', function(e) {
            e.preventDefault();
            let case_id = $(this).attr('data-row');
            var url = "<?php echo e(route('admin.case.viewForm.modify','CASE_ID')); ?>";
            url = url.replace('CASE_ID', case_id);
            $.ajax({
                url: url,
                type: 'GET',
...
<script>
        $(document).on('click', '.cloneCase', function() {
            if (confirm('Are you sure to clone this case')) {
                let case_id = $(this).attr('data-row');
                var url = "<?php echo e(route('admin.case.clone','CASE_ID')); ?>";
                url = url.replace('CASE_ID', case_id);
...
<script>
        $(document).on('click', '.HoldCase', function() {
            if (confirm('Are you sure to hold this case')) {
                let case_id = $(this).attr('data-row');
                var url = "<?php echo e(route('admin.case.hold','CASE_ID')); ?>";
                url = url.replace('CASE_ID', case_id);
...
<script>
        $(document).on('click', '.CaseDelete', function() {
            if (confirm('Are you sure to Delete this case')) {
                let case_id = $(this).attr('data-row');
                var url = "<?php echo e(route('admin.case.delete','CASE_ID')); ?>";
                url = url.replace('CASE_ID', case_id);
...
$(document).on('click', '.zoomimg', function(){
  ...
  $('#img01').attr('src', '<?php echo e(asset('')); ?>'+imgname);
  $("#myimgModal").fadeIn();
})
...
function fetchCases(page = 1, is_pagination = false) {
    ...
    $.ajax({
        url: `<?php echo e(route('admin.case.caseStatus', ['status' => '__STATUS__', 'user_id' => '__USERID__'])); ?>`.replace('__STATUS__', status).replace('__USERID__', user_id),
        type: 'GET',
        data: { 
            search: keyword, 
            perPage: perPage, 
...
<script>
    $(document).ready(function() {
        ...
        $('#gl_updateGeoLimitButton').on('click', function(e) {
            ...
            var url = "<?php echo e(route('admin.case.geolimit','CASE_ID')); ?>";
            url = url.replace('CASE_ID', caseTableId);
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('backend.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\kyc-live\resources\views/backend/pages/cases/caseList.blade.php ENDPATH**/ ?>